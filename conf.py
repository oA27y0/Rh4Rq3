...
def conf():
    ...
    ap.add_argument('--mode', default='rl-taxo', choices=['rl-taxo', 'sl', 'step-sl', 'hmcn'])
    ap.add_argument('--base_model', default='ohcnn-bow-fast', choices=['han', 'textcnn', 'ohcnn-bow-fast', 'raw'])
    ap.add_argument('--dataset', default='rcv1', choices=['yelp', 'rcv1', 'nyt', 'cellcycle_FUN', 'cellcycle_GO'])
    ap.add_argument('--isTrain', default=False, help='True for continuing training')
    ap.add_argument('--load_model', default=None)
    # Note: many of following parameters do not need any changes and are listed here just for convenience
    ap.add_argument('--word_gru_hidden_size', default=50)
    ap.add_argument('--sent_gru_hidden_size', default=50)
    ap.add_argument('--use_l1', default=True, help='if false, class_embed_size has to be size 2 * sent_gru')
    ...
    ap.add_argument('--allow_stay', default=True, help='always True')
    ...
    ap.add_argument('--mix_flat_probs', default=False, help='[experimental]add flat prob to help rl')
    ...
    ap.add_argument('--min_mem', default=6000, help='minimum gpu memory requirement (MB)')
    ap.add_argument('--batch_size', default=32)
    ap.add_argument('--batch_size_test', default=32)
    # outdated parameters
    ap.add_argument('--hist_embed_size', default=50, help='not used anymore')
    ...
    ap.add_argument('--filter_ancestors', default=False, help='not used anymore')
    ...
    elif args.dataset == 'lshtc':
        args.ohcnn_data = '_lshtc_root_padded'
        args.n_steps_sl = 10
        args.n_steps = 3  # TODO
        args.output_every = 3500
    ...
    if not args.use_l1:
        args.class_embed_size = args.sent_gru_hidden_size * 2
        args.use_cur_class_embed = False
    return args
