...
class Tree:
    def __init__(self, args, train_ids, test_ids, id2doc=None, id2doc_a=None, nodes=None, X_train=None,
        ...
        if args.mode != 'sl' and args.mode != 'hmcn':
            self.next_true_bin, self.next_true = self.generate_next_true(
                keep_cur=True)
        self.logger.info('{} terms have more than 1 parent'.format(
            sum([len(v) > 1 for v in self.c2p.values()])))
        ...
        if args.stat_check:
            ...
            # self.doc_check(self.train_ids, 'train')
            # self.doc_check(self.test_ids, 'test')
            ...
            if self.args.allow_stay:
                ...
                if self.args.allow_up:
                    if row_i in self.c2p_idx:
                        res[row_i, len(self.p2c_idx[row_i]) +
                            1] = self.c2p_idx[row_i][0]
            # next level node
    ...
    def doc_check(self, docids, name):
        ...
        for docid in docids:
            ...
            h = [self.id2h[self.name2id[term]]
                 for term in self.id2doc[docid][label_key]]
            hs.append(h)
        ...
        if self.args.stat_check:
            self.logger.info(
                'check info of classes for each doc in {}'.format(name))
    ...
    def get_next_by_probs(self, cur_class_batch, next_classes_batch, doc_ids, probs, save_prob):
        assert len(cur_class_batch) == len(
            doc_ids) == len(doc_ids) == len(probs)
        indices = []
    ...
    def calc_f1(self, pred_l, id_l, save_path=None, output=True):
        ...
        if not output:
            ...
            y_l_a = [self.id2doc_ancestors[docid]['class_idx']
                     for docid in id_l]
            ...
            self.logger.info(
                f'micro:{f1_a:.4f} macro:{f1_a_macro:.4f} samples:{f1_a_s:.4f}')
    ...
    def read_mapping(self):
        ...
        for bid in self.id2doc:
            self.id2doc[bid]['categories'] = [
                c for c in self.id2doc[bid]['categories'] if c != self.rootname]
            self.id2doc[bid]['class_idx'] = [self.id2idx[c]
                                             for c in self.id2doc[bid]['categories']]
        ...
        for bid in self.id2doc_ancestors:
            self.id2doc_ancestors[bid]['categories'] = [c for c in self.id2doc_ancestors[bid]['categories'] if
                                                        c != self.rootname]
            self.id2doc_ancestors[bid]['class_idx'] = [self.id2idx[c]
                                                       for c in self.id2doc_ancestors[bid]['categories']]
