...
from util import get_gpu_memory_map, save_checkpoint, check_doc_size, gen_minibatch_from_cache, gen_minibatch, \
    save_minibatch, contains_nan, explode
...
def train_step_sl():
    ...
    for i in range(1, args.num_epoch + 1):
        ...
        for ct, (tokens, doc_ids) in tqdm(enumerate(g)):
            ...
            global_loss, flat_probs = forward_step_sl(tokens, doc_ids)
            optimizer.zero_grad()
        ...
        if i % args.save_every == 0:
            eval_save_model(i, datapath='train' + args.ohcnn_data, save=True)
...
def eval_save_model(i, pred_l=None, target_l=None, datapath=None, save=False, output=True):
    ...
    if pred_l:
        ...
        f1, f1_a, f1_aa, f1_macro, f1_a_macro, f1_aa_macro, f1_aa_s = test_f(datapath)
    else:
...
if args.load_model:
    if os.path.isfile(args.load_model):
        ...
        if args.mode in ['sl', 'hmcn']:
            ...
            if checkpoint['state_dict']['class_embed.weight'].size()[0] == \
